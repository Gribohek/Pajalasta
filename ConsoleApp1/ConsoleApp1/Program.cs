...
namespace ConsoleApp1
{
    class Program
    {
        ...
        async static Task Update(ITelegramBotClient botClient, Update update, CancellationToken arg3)
        {
            ...
            if (message.Text.ToLower().Contains("конечно")||(message.Text.ToLower().Contains("да")))
            {
                {
            ...
            if (message.Text.ToLower().Contains("в первую"))
            {
                ...
                await botClient.SendTextMessageAsync(message.Chat.Id, "Dнутреннее чувство подсказывает тебе идти в первую комнату", replyMarkup: new ReplyKeyboardMarkup(KB));
                return;


            if (message.Text.ToLower().Contains("свечу"))
            {
                KB.Add(new KeyboardButton("Свечу"));
                KB.Add(new KeyboardButton("Зеркало"));
                KB.Add(new KeyboardButton("Ключ"));
                KB.Add(new KeyboardButton("Меч"));
                await botClient.SendTextMessageAsync(message.Chat.Id, "персонаж подходит и зажигает свечу, но ничего не происходит и свеча тут же тухнет", replyMarkup: new ReplyKeyboardMarkup(KB));
                KB.Clear();
                return;
            }
            if (message.Text.ToLower().Contains("зеркало"))
            {
                KB.Add(new KeyboardButton("Свечу"));
                KB.Add(new KeyboardButton("Зеркало"));
                KB.Add(new KeyboardButton("Ключ"));
                KB.Add(new KeyboardButton("Меч"));
                await botClient.SendTextMessageAsync(message.Chat.Id, "персонаж подбирает зеркало, осматривает себя в нём и кладёт обратно", replyMarkup: new ReplyKeyboardMarkup(KB));
                KB.Clear();
                return;
            }
            if (message.Text.ToLower().Contains("меч"))
            {
                KB.Add(new KeyboardButton("Свечу"));
                KB.Add(new KeyboardButton("Зеркало"));
                KB.Add(new KeyboardButton("Ключ"));
            ...
            if (message.Text.ToLower().Contains("ключ"))
            {
                KB.Add(new KeyboardButton("Свечу"));
                KB.Add(new KeyboardButton("Зеркало"));
                KB.Add(new KeyboardButton("Ключ"));
            ...
            if (message.Text.ToLower().Contains("остаться в комнате"))
            {
                KB.Add(new KeyboardButton("Свечу"));
                KB.Add(new KeyboardButton("Зеркало"));
                KB.Add(new KeyboardButton("Ключ"));
                KB.Add(new KeyboardButton("Меч"));
                await botClient.SendTextMessageAsync(message.Chat.Id, "В ходе осмотра он находит письмо в котором сказано, что нужно взять зеркало с собой", replyMarkup: new ReplyKeyboardMarkup(KB));
                KB.Clear();
                return;
            }
            if (message.Text.ToLower().Contains("перейти в следующую комнату"))
            {
                await botClient.SendTextMessageAsync(message.Chat.Id, "Пользователь переходит в следующую комнату, где находит записку. Записка: Чтобы разгадать следующую головоломку, найди ключ к соединению небесных тел.", replyMarkup: new ReplyKeyboardMarkup(KB));
            }

            if (message.Text.ToLower().Contains("посмотреть вверх"))
            {
            ...
            {
                await botClient.SendTextMessageAsync(message.Chat.Id, "Пользователь заходит в конечную комнату и встречает босса. Ты смело преодолел все преграды, но перед тобой стою я - босс этого замка. Готов ли ты сразиться со мной?", replyMarkup: new ReplyKeyboardMarkup(KB));
            }
